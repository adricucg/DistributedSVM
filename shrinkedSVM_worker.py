...
def shrinked_svm_worker(sess, Y, X, gradient, alpha, worker_index, active_indeces, x_i_up, x_i_low, v_up, v_low, C, no_shrinks):
    ...
    Y_active = \
        tf.get_variable('Y_active_' + str(worker_index) + '_' + str(no_shrinks), initializer=tf.gather(Y, active_indeces))
    X_active = \
        tf.get_variable('X_active_' + str(worker_index) + '_' + str(no_shrinks), initializer=tf.gather(X, active_indeces))
    gradient_active = \
        tf.get_variable('g_active_' + str(worker_index) + '_' + str(no_shrinks), initializer=tf.gather(gradient, active_indeces))
    alpha_active = \
        tf.get_variable('alpha_active_' + str(worker_index) + '_' + str(no_shrinks), initializer=tf.gather(alpha, active_indeces))

    sess.run(tf.global_variables_initializer())

    sess.run([Y_active, X_active, gradient_active, alpha_active])
...
def prepare_shrinked_svm_workers(sess,
        Y_list, X_list, gradient_list, alpha_list,
        p, active_indeces_list, x_i_up, x_i_low, v_up, v_low, C, no_shrinks):
    ...
    for k in range(0, p):
        with tf.device("/job:job1/task:" + str(k)):
            ...
            min_gradient, max_gradient, x_iup, x_ilow, target_i_up, target_i_low, i_up, i_low \
                = shrinked_svm_worker(sess,
                Y_list[k], X_list[k], gradient_list[k], alpha_list[k],
                k, active_indeces_list[k], x_i_up, x_i_low, v_up, v_low, C, no_shrinks)
...
def update_gradient_and_alpha(p, active_indeces_list, no_shrinks):
    ...
    for k in range(0, p):
        ...
        active_indeces = active_indeces_list[k]
        ...
        with tf.device("/job:job1/task:" + str(k)):
            ...
            with tf.variable_scope("svm_worker_active", reuse=True) :
                gradient_active = tf.get_variable('g_active_' + str(k) + '_' + str(no_shrinks))
                alpha_active = tf.get_variable('alpha_active_' + str(k) + '_' + str(no_shrinks))

            update_index = tf.reshape(active_indeces, shape=[len(active_indeces), 1])
            gradient_update = tf.scatter_nd_update(gradient, update_index, gradient_active)
            alpha_update = tf.scatter_nd_update(alpha, update_index, alpha_active)

        g_update_list.append(gradient_update)
        alpha_update_list.append(alpha_update)

    output = [g_update_list, alpha_update_list]
